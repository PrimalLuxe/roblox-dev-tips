...
for marker in ['Discoveries = 5','TotalShakes = 12','HighestRarity = "Epic"']:
    if marker not in machine_text: err(f'machine progression missing {marker}')
...
for marker in ['BUILDING_STYLES','storefront(hub','RoadNorthSouth','RoadEastWest','DowntownDirectory','MachineArtDirector.Apply']:
    if marker not in world: err(f'authored Downtown marker missing: {marker}')
...
if 'fillFacades' in polish or 'CornerStoreBuilding' in polish: err('free-model storefront architecture returned')
loading=read('ReplicatedFirst/Loading.client.lua')
if 'UIListLayout' in loading or 'OLYMPUS ENTERTAINMENT' not in loading: err('Olympus intro can scramble again')
...
print('  mutations:',len(mutations-{'None'}),' + None')
