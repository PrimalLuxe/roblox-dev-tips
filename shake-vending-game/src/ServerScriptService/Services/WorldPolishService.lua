...
local function styleBillboard(machine)
    ...
    frame.BackgroundColor3=Color3.fromRGB(255,248,226);frame.BackgroundTransparency=.03
    local title=frame:FindFirstChild("Title");local subtitle=frame:FindFirstChild("Subtitle")
    if title then title.TextColor3=Color3.fromRGB(31,49,70);title.TextStrokeTransparency=1 end
    if subtitle and machine:GetAttribute("EventOnly")then subtitle.TextColor3=Color3.fromRGB(193,121,58)end
...
local function polishMachine(machine)
    if machine:GetAttribute("ArtDirectionVersion")==2 then return end
    ...
    local hl=shell:FindFirstChild("MachineAccent");if hl and hl:IsA("Highlight")then hl.OutlineTransparency=.9 end
    local tray=art and art:FindFirstChild("DispenseTray");local spawn=machine:FindFirstChild("DropSpawn");if tray and spawn then spawn.CFrame=CFrame.new(tray.Position+Vector3.new(0,math.max(.45,tray.Size.Y*.5+.35),0))end
end
...
function WorldPolishService:Init()
    ...
    local hub=Workspace:FindFirstChild("ShakeHub");if not hub then return end;fillDecor(hub)
    -- Warm Creator Store donors load asynchronously. Retry only the missing dressing; never block server ready.
    task.spawn(function()for _=1,6 do task.wait(2);if not hub.Parent then return end;fillDecor(hub)end end)
end
