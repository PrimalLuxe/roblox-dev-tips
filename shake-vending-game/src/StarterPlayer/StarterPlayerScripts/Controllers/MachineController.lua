...
local function machineScreen(machine)
    local shell=machine:FindFirstChild("Shell")
    local art=shell and shell:FindFirstChild("OlympusKitbash")
    local screen=art and art:FindFirstChild("MachineScreen")
    local gui=screen and screen:FindFirstChild("ArtLabel")
    local frame=gui and gui:FindFirstChildOfClass("Frame")
    return frame and frame:FindFirstChildOfClass("TextLabel"),screen
end
...
function MachineController:SetSnapshot(snapshot)self.Snapshot=snapshot;self:UpdateDisplays(self.Event or(snapshot and snapshot.Event))end
...
function MachineController:UpdateDisplays(event)
    ...
    if event and event.Id=="Blackout"then overlay.Brightness=-.16;overlay.Contrast=.08;overlay.TintColor=Color3.fromRGB(205,190,255)elseif event and event.Id=="Glitch"then overlay.Saturation=.08;overlay.TintColor=Color3.fromRGB(239,218,255)end

    for _,machine in ipairs(folder:GetChildren())do
        local id=machine:GetAttribute("MachineId")or machine.Name;local def=Machines[id];if not def then continue end
        local applies=event and event.Id~="Normal"and(event.Global or event.TargetMachine==id)
        local accent=applies and(eventColors[event.Id]or def.Accent)or def.Accent
        local h=machine:FindFirstChild("Shell")and machine.Shell:FindFirstChild("MachineAccent");if h and h:IsA("Highlight")then h.OutlineColor=accent end
        local root=machine:FindFirstChild("Root");local prompt=root and root:FindFirstChild("ShakePrompt")
        local display,screen=machineScreen(machine)
        ...
        local status="READY\nSHAKE"
        if applies then
            status=event.Jammed and string.format("JAMMED\n%d / %d",event.JamCount or 0,event.JamTarget or 500)or(event.Hidden and"???\nRESTOCK"or string.upper(event.Display or"EVENT"))
        elseif def.EventOnly and not eventOnline then status="OFFLINE\nEVENT ONLY"
        elseif unlocked or(def.UnlockCost==0 and not def.EventOnly)then
            ...
            local nextMilestone;for _,m in ipairs(Config.MachineMasteryMilestones)do if shakes<m.Shakes then nextMilestone=m;break end end
            status=nextMilestone and string.format("SHAKE\nMASTERY %d/%d",shakes,nextMilestone.Shakes)or"MASTERED\nHUNT RARES"
        else
            local firstMissing;for _,req in ipairs(unlockState and unlockState.Requirements or{})do if not req.Met then firstMissing=req;break end end
            if firstMissing then status=string.format("LOCKED\n%s %s/%s",string.upper(firstMissing.Label),Util.FormatInteger(firstMissing.Current),Util.FormatInteger(firstMissing.Target))
            else status="READY\n$"..Util.FormatInteger(def.UnlockCost or 0)end
        end
        if display then display.Text=status;display.TextColor3=Color3.fromRGB(22,34,44)end
        if screen then screen.Color=accent:Lerp(Color3.fromRGB(115,221,238),.55)end
        if prompt then
...
function MachineController:Init(remotes)
    ...
    for _,m in ipairs(folder:GetChildren())do self:ConnectMachine(m)end
    folder.ChildAdded:Connect(function(m)task.wait();self:ConnectMachine(m);self:UpdateDisplays(self.Event)end)
